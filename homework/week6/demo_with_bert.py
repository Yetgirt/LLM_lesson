...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x = self.classify(pooler_output)    # pooler_output : (B, 768)
...
def build_sample(vocab, sentence_length):
    # 步骤1：随机生成一个长度为 sentence_length 的样本（字符可能重复）
    x = [random.choice(list(vocab.keys())) for _ in range(sentence_length)]

    # 步骤2：把样本转成集合，去重后方便判断是否包含某些字符
    x_set = set(x)  # 例如：{'a', 'b', 'd', 'f'} 或 {'x', 'y', 'z'} 等

    # 判断规则（互斥的三类）：
    # A类：必须包含 abc 中的至少一个，但不能出现 xyz 中的任何一个
    if (x_set & {"a", "b", "c"}) and not (x_set & {"x", "y", "z"}):
        y = 0  # A类：有abc，无xyz

    # B类：必须包含 xyz 中的至少一个，但不能出现 abc 中的任何一个
    elif not (x_set & {"a", "b", "c"}) and (x_set & {"x", "y", "z"}):
        y = 1  # B类：无abc，有xyz

    # C类：其他所有情况（两种都有、两种都没有、只有部分）
    else:
        y = 2  # C类：混合或都没有

    # 步骤3：把字符转成 vocab 中的 id（虽然这里不会出现 unk，但保险起见）
