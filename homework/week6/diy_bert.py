...
class DiyBert:
    ...
    def self_attention(self,
        ...
        qk = np.matmul(q, k.swapaxes(1, 2))  #在遇到 ≥2 维 的张量时，自动只对最后两维相乘，前面的维度自动 batch 广播。 批矩阵乘法”（batched matrix multiply
        qk /= np.sqrt(attention_head_size)
        qk = softmax(qk)
        # qkv.shape = num_attention_heads, max_len, attention_head_size
        qkv = np.matmul(qk, v)
        # qkv.shape = max_len, hidden_size
        qkv = qkv.swapaxes(0, 1).reshape(-1, hidden_size) #-1：自动计算，总元素大小/hidden_size。reshape是把所有元素展平，再按照hidden_size为一组收集，那么最先组合的就是后面两维，只要hidden_size = 后面两维之积，那么张量的第一维就是文本长度。
