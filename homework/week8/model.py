...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, sam_a, sam_p=None, sam_n=None):
        #同时传入两个句子
        if sam_p is not None:
            vector1 = self.sentence_encoder(sam_a) #vec:(batch_size, hidden_size)
            vector2 = self.sentence_encoder(sam_p)
            #如果有标签，则计算loss
            if sam_n is not None:
                # return self.loss(vector1, vector2, target.squeeze())
                vector3 = self.sentence_encoder(sam_n)
        ...
        #单独传入一个句子时，认为正在使用向量化能力
            ...
            return self.sentence_encoder(sam_a)
